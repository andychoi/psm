...
def qs_to_df(model, qs=None, fields=None, exclude=None, date_cols=None, **kwargs):
    """
    ...
    """
        how to get foreign names, many-to-many also works...
        df2 = qs_to_df(model=Project, qs=Project.objects.all(), fields=['type', 'dept__name', 'counts'])
        proj_stat = Chart('stackedBar', chart_id='proj_stat', palette=PALETTE)
        proj_stat.from_df(df2, values='counts', stacks=['type'], labels=['dept__name'])
    """
    ...
    if qs:
        records = qs.filter(**kwargs).values_list(*fields)
    else:
