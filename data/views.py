...
class Dashboard(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        df2 = qs_to_df(model=Project, qs=Project.objects.filter(year=2022), 
            date_cols=['%Y-%m', 'p_launch'],
            fields=['type', 'dept__name', 'CBUs__name', 'counts', 'p_launch'])

        proj_stat1 = Chart('stackedBar', chart_id='proj_stat1', palette=PALETTE)
        proj_stat1.from_df(df2, values='counts', stacks=['type'], labels=['CBUs__name'])
        context['charts'].append(proj_stat1.get_presentation())

        proj_stat2 = Chart('stackedBar', chart_id='proj_stat2', palette=PALETTE)
        proj_stat2.from_df(df2, values='counts', stacks=['type'], labels=['p_launch'])
